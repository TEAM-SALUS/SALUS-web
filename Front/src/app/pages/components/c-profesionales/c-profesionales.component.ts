...
export class CProfesionalComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.especialidadService
      .getEspecialidadId(this.especialidadId)
      .subscribe((especialidad) => {
        this.especialidadLista = [especialidad];
        this.profesionalService
          .getProfesionalesByEspecialidad(especialidad.id)
          .subscribe((profesionales) => {
            this.profesionalLista = profesionales.filter(
              (profesional) => profesional.id_especialidad == especialidad.id
            );
    ...
    this.profesionalService
      .getProfesionalId(this.profesionalId)
      .subscribe((profesional) => {
        this.profesionalLista = [profesional];
        if (profesional.id_horario !== undefined) {
          this.horarioService
            .getProfesionalXHorario(profesional.id_horario)
            .subscribe((horarios) => {
              this.horarioAtencionLista = horarios.filter(
                (horario) => horario.id == profesional.id_horario
              );
  ...
  public obtenerProfesionales() {
    this.profesionalService.getProfesionales().subscribe((respuesta) => {
      this.profesionalLista = respuesta;
      console.log("PROFESIONAL",respuesta);
    });
  }
}
